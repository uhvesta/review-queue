...
scan_file() {
  local file="$1"
  if [[ "${REVIEW_QUEUE_SCAN_FORCE_GREP:-0}" != "1" ]] &&
    command -v rg >/dev/null 2>&1; then
    if LC_ALL=C strings -a "$file" 2>/dev/null |
      rg --no-line-number --no-filename "$credential_pattern" >/dev/null; then
      echo "release secret scan failed: possible credential material in $file" >&2
      return 1
    fi
  elif LC_ALL=C strings -a "$file" 2>/dev/null |
    grep -E -- "$credential_pattern" >/dev/null; then
    echo "release secret scan failed: possible credential material in $file" >&2
    return 1
  fi
}
