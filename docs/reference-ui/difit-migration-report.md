...
- The current tree was rebuilt and exercised again in a native Tauri window.
  Queue Home, continuous review, inline conversations, Unified/Split, global
  hunk navigation, and Full file all rendered with the new state-isolation
  and keyboard fixes. A Keychain ACL wait no longer blocks the window because
  connection operations run off the main thread. Current screenshots:
  `../evidence/v0.1.0/difit-current-queue.jpeg`,
  `../evidence/v0.1.0/difit-current-reviewer.jpeg`,
  `../evidence/v0.1.0/difit-current-split.jpeg`, and
  `../evidence/v0.1.0/difit-current-full-file.jpeg`.
